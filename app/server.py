...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.exception("failed to load IndexTTS2 model")
        raise
    yield


app = FastAPI(
    title="IndexTTS OpenAI-TTS API",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def create_speech(body: SpeechRequest, request: Request):
    ...
    try:
        samples = await engine.synthesize_clone(
            text,
            spk_wav=str(voice.wav_path),
            params=body.generation,
        )
...
async def create_emotion(body: EmotionRequest, request: Request):
    settings = request.app.state.settings
    engine: TTSEngine = request.app.state.engine
    catalog: VoiceCatalog = request.app.state.catalog
    ...
    try:
        if body.emotion_voice is not None:
            emo_voice = _require_voice(catalog, body.emotion_voice)
            samples = await engine.synthesize_emotion_audio(
                text,
                spk_wav=str(voice.wav_path),
                emo_wav=str(emo_voice.wav_path),
                emo_alpha=body.emotion_alpha,
                params=body.generation,
            )
        elif body.emotion_vector is not None:
            samples = await engine.synthesize_emotion_vector(
                text,
                spk_wav=str(voice.wav_path),
                emo_vector=body.emotion_vector,
                use_random=body.use_random,
                params=body.generation,
            )
        else:
            emo_text = (body.emotion_text or "").strip()
            if not emo_text:
                raise HTTPException(status_code=422, detail="emotion_text is empty")
            samples = await engine.synthesize_emotion_text(
                text,
                spk_wav=str(voice.wav_path),
                emo_text=emo_text,
                emo_alpha=body.emotion_alpha,
